...
const validatePackageScriptsPath = resolve(root, 'scripts/validate-package-scripts.mjs');
...
const dockerfileOpenclaw = existsSync(dockerfileOpenclawPath)
  ? readFileSync(dockerfileOpenclawPath, 'utf8')
  : '';
...
const agentDealContext = existsSync(agentDealContextPath)
  ? readFileSync(agentDealContextPath, 'utf8')
  : '';
...
const applySupabaseMigrations = existsSync(applySupabaseMigrationsPath)
  ? readFileSync(applySupabaseMigrationsPath, 'utf8')
  : '';
const validatePackageScripts = existsSync(validatePackageScriptsPath)
  ? readFileSync(validatePackageScriptsPath, 'utf8')
  : '';
...
const netlifyPublicAvaFunction = existsSync(netlifyPublicAvaFunctionPath)
  ? readFileSync(netlifyPublicAvaFunctionPath, 'utf8')
  : '';
const netlifyDocumentsPdfFunction = existsSync(netlifyDocumentsPdfFunctionPath)
  ? readFileSync(netlifyDocumentsPdfFunctionPath, 'utf8')
  : '';
const netlifyBridgeProxyFunction = existsSync(netlifyBridgeProxyFunctionPath)
  ? readFileSync(netlifyBridgeProxyFunctionPath, 'utf8')
  : '';
...
const netlifyBrainCleanRouteOrdered =
  netlifyBrainCleanIndex >= 0 &&
  netlifyBrainProxyIndex >= 0 &&
  netlifyBrainCleanIndex < netlifyBrainProxyIndex &&
  /from\s*=\s*"\/brain"[\s\S]*?to\s*=\s*"\/index\.html"[\s\S]*?status\s*=\s*200/.test(
    netlifyConfig.slice(netlifyBrainCleanIndex, netlifyBrainProxyIndex)
  );
const cleanPathRouterWired =
  /const\s+cleanPagePathAliases\s*=/.test(index) &&
...
const emotionWorldModelSmoke = existsSync(resolve(root, 'scripts/emotion-world-model-smoke.mjs'))
  ...
  : '';
const emotionWorldModelTrainingSmoke = existsSync(
  resolve(root, 'scripts/emotion-world-model-training-smoke.mjs')
)
...
const productionPristineCheck = existsSync(resolve(root, 'scripts/production-pristine-check.mjs'))
  ...
  : '';
const emotionOnnxExporter = existsSync(emotionOnnxExporterPath)
  ? readFileSync(emotionOnnxExporterPath, 'utf8')
  : '';
const emotionRequirements = existsSync(emotionRequirementsPath)
  ? readFileSync(emotionRequirementsPath, 'utf8')
  : '';
const defaultAgentFleetBlock =
  bridge.match(/function buildDefaultAgentFleet\(\) \{[\s\S]*?\n\}/)?.[0] || '';

const checks = [
  {
    name: 'Brain page uses operator-friendly language for core research actions',
    ok:
      /PBK Brain - research, scripts, memory, and market notes/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /function\s+renderLeadDetail/.test(index) &&
      /function\s+openLeadEditModal/.test(index) &&
      /PATCH/.test(bridge) &&
      /\/api\/leads\/:id/.test(bridge),
  },
  {
    name: 'Unified inbox is seller/homeowner facing only and lead-edit aware',
    ok:
      /function\s+isSellerFacingInboxMessage/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /getLeadEditDefaultsWithCallContext/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /loadActiveLeadIntoAnalyzer/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /leadAddressKey\s*=\s*slugify/.test(bridge) &&
      /leadPhoneKey\s*=\s*normalizePhone/.test(bridge) &&
      /leadEmailKey/.test(bridge) &&
      /Boolean\(leadAddressKey && leadPhoneKey/.test(bridge) &&
      !/dedupeKey\s*=\s*`\$\{slugify\(leadImport\.property\.address\)\}::\$\{normalizePhone\(leadImport\.seller\.phone\)\}`/.test(
        bridge
      ),
  },
  {
    ...
    ok:
      /contractStageFilter/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /matchPath\(pathname,\s*'\/api\/messages\/:id'\)/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /valuePayload\s*=\s*JSON\.parse/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /HOSTED_BRIDGE_AUTH_MISCONFIGURED/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /const includeState = \['1', 'true', 'yes'\]\.includes/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /pathname === '\/api\/calls'[\s\S]*?const limit = Math\.max\(1, Math\.min\(200, Number\(url\.searchParams\.get\('limit'\)/.test(
        bridge
      ) &&
      /calls: allCalls\.slice\(offset, offset \+ limit\)/.test(bridge) &&
      /pathname === '\/api\/messages'[\s\S]*?const limit = Math\.max\(1, Math\.min\(200, Number\(url\.searchParams\.get\('limit'\)/.test(
        bridge
      ) &&
  ...
  },
  {
    ...
    ok:
      ...
      /looksLikeRexDirectedCommand/.test(bridge) &&
      /stripRexInvocation/.test(bridge) &&
      /routedTo = 'rex_conversational_brain'/.test(bridge) &&
      /toolHandlers\.getBrainState\(\{[\s\S]*agent: 'rex'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /TOOL_FIRST_INTENT_TO_TOOL/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /CREATE TABLE IF NOT EXISTS public\.pbk_tool_usage/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      ...
      /withButtonLoading\(\s*button,[\s\S]*normalizeRuntimeStatus\(action\) === 'cancelled' \? 'Cancelling\.\.\.' : 'Sending\.\.\.'/.test(
        index
      ),
  },
  {
    ...
    ok:
      /function\s+shouldShowApprovalBantCheck/.test(index) &&
      /contract\|docusign\|doc\|outbound\|campaign\|call\|sms\|email\|admin\|schema\|settings/.test(
        index
      ) &&
  ...
  },
  {
    ...
    ok:
      /function\s+saveCampaignLeadSelector/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /function\s+renderRecordingsLibrary/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /function isMetadataOnlySmokeRecording/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /deleteRuntimeRecording/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /function\s+startOpenClawRealtime/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /OPENCLAW_GATEWAY_WS_HANDSHAKE_TIMEOUT_MS/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /IS_HOSTED\s*\?\s*''\s*:\s*'ws:\/\/127\.0\.0\.1:18789'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function shouldReturnGatewayStatusOk/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function isOpenClawDirectGatewayConfigured/.test(bridge) &&
      /connectionMode/.test(bridge) &&
      /heartbeat_only/.test(bridge) &&
      !/bridge keeps retrying/.test(bridge) &&
      /OpenClaw gateway \$\{gatewayStatus\.ready \? 'healthy' : gatewayStatus\.directProbeConfigured \? 'direct probe unavailable' : 'standby'\}/.test(
        index
      ),
  },
  {
  ...
  },
  {
    ...
    ok:
      /BatchData optional\/off/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /action === 'train_source'/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /queued skill transfer retry failed/.test(agentFleet) &&
  ...
  },
  {
    ...
    ok:
      /function buildDefaultAgentFleet/.test(bridge) &&
      /activity:\s*'Waiting for approved PBK work.'/.test(defaultAgentFleetBlock) &&
      /Ready for approval-gated offer recap, seller follow-up, and contract delivery skills/.test(
        defaultAgentFleetBlock
      ) &&
      ...
      !/id:\s*'nora'|id:\s*'zed'|Diane Kowalski|Probate Warm-up Q2|On call with Diane|Spanish acquisitions|Outbound SMS/i.test(
        defaultAgentFleetBlock
      ),
  },
  {
    ...
    ok:
      /function buildAgentOrchestrationSnapshot/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /async function runAgentOrchestrationSmoke/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /Browser microphone streaming is disabled/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /ELEVENLABS_STREAMING_TTS_ENABLED/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function buildAvaFlowTurn/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /detectAvaJarvisCommand/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /AVA_MASTERCLASS_KNOWLEDGE_REVISION/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function buildClosingIntelligenceAdvice/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function buildAvaConversationIntelligence/.test(bridge) &&
      ...
      /function recordPostCallLearningFromTranscript/.test(bridge) &&
      ...
      /post_call_learning_completed/.test(bridge) &&
      /sourceOutcomeTable:\s*'pbk_skill_outcomes'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function runTavilySearch/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function looksLikeAvaTtsDiagnosticCommand/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /let decisionResponse = null/.test(index) &&
      /decisionResponse = await withButtonLoading\([\s\S]*requestOpenClawApi\(approvalPath/.test(
        index
      ) &&
      /decisionResponse = await withButtonLoading\([\s\S]*updateAdminTaskDecision/.test(index) &&
      /decisionResponse\?\.state[\s\S]*renderOpenClawState\(decisionResponse\.state\)/.test(
        index
      ) &&
  ...
  },
  {
    ...
    ok:
      /function getWebSearchProviderStatus/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function createLifNetwork/.test(snnCore) &&
  ...
  },
  {
    ...
    ok:
      /CREATE TABLE IF NOT EXISTS public\.pbk_agent_decisions/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /CREATE TABLE IF NOT EXISTS public\.pbk_call_emotions/.test(bridge) &&
      ...
      /function runNativeEmotionWorldModelProvider/.test(bridge) &&
      /modelProvider:\s*'external_world_model'/.test(bridge) &&
      /modelProvider:\s*'native_trained_model'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function buildXFactorCapabilitySnapshot/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function renderApprovalBridgeKeyRequired/.test(index) &&
      /textContent = '- Locked'/.test(index) &&
      /textContent = `\$\{label\} Locked`/.test(index) &&
      /function renderRuntimeLoadingState/.test(index) &&
      /openClawNeedsBridgeApiKey\(config, '\/state'\)[\s\S]*renderApprovalBridgeKeyRequired/.test(
        index
      ),
  },
  {
    ...
    ok:
      /function saveOpenClawConfig\(partial = \{\}, options = \{\}\)/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /function buildAgentStatusBundle/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function stripNegatedActionInstructions/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_PATH_SCRIPT_TRIGGERS/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_100K_WAR_MANUAL_REVISION/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_ACTIVE_LISTENING_REVISION/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_CALLER_ROLE_REVISION/.test(bridge) &&
      ...
      /are you the property owner, or are you a real estate agent representing the seller/.test(
        bridge
      ) &&
      /Creative Finance and Multi-Family are agent-only/.test(bridge) &&
      /Ava did not reassure the agent that commission stays protected before probing/.test(
        openclawSmoke
      ) &&
  ...
  },
  {
    ...
    ok:
      /PBK_AVA_FULL_INTELLIGENCE_REVISION/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function buildAvaCallStateSummary/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /runOpenAiWebSearch\(query, \{ \.\.\.params, fallback: false \}\)/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      ...
      /Inject-transcript debug endpoint did not safely report a missing active media session/.test(
        openclawSmoke
      ) &&
  ...
  },
  {
    ...
    ok:
      /createClient as createRedisClient/.test(bridge) &&
      ...
      /Voice status endpoint did not expose optional Redis shared-state diagnostics/.test(
        openclawSmoke
      ),
  },
  {
    ...
    ok:
      /production-pristine-check\.mjs/.test(pkg) &&
  ...
  },
  {
    ...
    ok:
      /key:\s*PBK_TAVILY_API_KEY/.test(renderConfig) &&
  ...
  },
  {
    ...
    ok:
      /retrieveClosingIntelligenceRequest/.test(runtimeBridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_DEEPGRAM_BROWSER_LIVE_MODEL/.test(bridge) &&
      ...
      /listenVersion:\s*isDeepgramFluxModel\(normalizedModel\)\s*\?\s*'v2'\s*:\s*'v1'/.test(
        bridge
      ) &&
      ...
      /model\s*===\s*'nova-2-general'\s*\|\|\s*model\s*===\s*'nova-2-meeting'\)\s*return\s*'nova-2'/.test(
        bridge
      ) &&
      ...
      /createManualDeepgramLiveConnection/.test(
        readFileSync(resolve(root, 'scripts/pbk-deepgram-client.mjs'), 'utf8')
      ) &&
      /Authorization:\s*`Token \$\{config\.apiKey\}`/.test(
        readFileSync(resolve(root, 'scripts/pbk-deepgram-client.mjs'), 'utf8')
      ),
  },
  {
    ...
    ok:
      /memoryType:\s*'call_transcript'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /persistPbkMemoryToSupabaseRest/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function\s+estimatePbkLiveSentiment/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /requestPayload\.stream_auth_token\s*=\s*TELNYX_MEDIA_STREAM_TOKEN/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /Telnyx media WebSocket connected to PBK bridge/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /buildTelnyxVoiceRoutingDiagnostic/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /TELNYX_DEEPGRAM_OPEN_TIMEOUT_MS/.test(bridge) &&
      /pendingTelnyxMediaFrames/.test(bridge) &&
      /flushPendingTelnyxMediaFrames/.test(bridge) &&
      /withTimeout\(deepgramConnection\.waitForOpen\(\),\s*TELNYX_DEEPGRAM_OPEN_TIMEOUT_MS/.test(
        bridge
      ) &&
  ...
  },
  {
    ...
    ok:
      /function\s+isTelnyxMessageWebhook/.test(bridge) &&
      /telnyxWebhookPath && !isTelnyxInboundCallWebhook\(body\)/.test(bridge) &&
      /mappedEvent:\s*mapped\.eventType/.test(bridge) &&
      /webhookType:\s*isTelnyxMessageWebhook\(body\)\s*\?\s*'message'\s*:\s*'non-call'/.test(
        bridge
      ),
  },
  {
  ...
  },
  {
    ...
    ok:
      /PBK_INBOUND_AFTER_HOURS_VOICEMAIL_ENABLED/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_DEEPGRAM_TELNYX_LIVE_MODEL/.test(bridge) &&
      ...
      /utteranceEndMs:\s*1000/.test(bridge) &&
      /Unexpected server response: \$\{response\.statusCode\}\$\{body/.test(
        readFileSync(resolve(root, 'scripts/pbk-deepgram-client.mjs'), 'utf8')
      ),
  },
  {
    ...
    ok:
      /Deepgram media stream ended without a final transcript/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function\s+decodeG711FrameToLinear16/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /TELNYX_DEEPGRAM_KEEPALIVE_MS/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /sendDeepgramControl/.test(bridge) &&
      /type:\s*'Finalize'/.test(bridge) &&
      /type:\s*'CloseStream'/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /outboundAvaGreetingSpoken/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function\s+looksLikeStrategistMetaText/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function\s+buildFastTelnyxLiveAvaReplyText/.test(bridge) &&
      ...
      /maxTokens:\s*(?:220|320)/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /lastAvaReplyTranscript/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /async function resolveAvaLiveCallContext/.test(bridge) &&
      /function buildAvaResolvedNextMove/.test(bridge) &&
      /const goalInference = architecture\.goalInference \|\| session\.goalInference \|\| conversation\.goalInference \|\| \{\}/.test(
        bridge
      ) &&
  ...
  },
  {
    ...
    ok:
      ...
      /async function ensureCallEmbeddingsSchema/.test(bridge) &&
      /CREATE TABLE IF NOT EXISTS public\.call_embeddings/.test(bridge) &&
      /CREATE OR REPLACE FUNCTION public\.match_call_embeddings/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /AVA_LIVE_RAG_ENABLED/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /AVA_GOAL_INFERENCE_GOALS/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_TELNYX_RECORD_INBOUND_CALLS/.test(bridge) &&
      ...
      !/after_hours_voicemail'[\s\S]{0,1400}action:\s*'record',\s*result:\s*await recordTelnyxCall/.test(
        bridge
      ),
  },
  {
  ...
  },
  {
    ...
    ok:
      /buildTelnyxLiveConversationalRepairReply/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /Array\.isArray\(value\)/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_TELNYX_ELEVENLABS_MEDIA_REPLY_ENABLED/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /startTelnyxMediaStream/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /function\s+buildPublicAvaLeadAnswer/.test(bridge) &&
  ...
  },
  {
    ...
    ok:
      /PBK_PUBLIC_AVA_NETLIFY_RATE_LIMIT_MAX/.test(netlifyPublicAvaFunction) &&
  ...
  },
  {
    ...
    ok:
      /X-Request-ID/.test(netlifyBridgeProxyFunction) &&
  ...
  },
  {
    ...
    ok:
      /function buildCorsHeaders/.test(netlifyBridgeProxyFunction) &&
  ...
  },
  {
    ...
    ok:
      /data-plugin-action="clear-key"/.test(index) &&
  ...
  },
  {
    ...
    ok:
      /export\s+const\s+handler/.test(netlifyDocumentsPdfFunction) &&
  ...
  },
  {
    ...
    ok:
      existsSync(validatePackageScriptsPath) &&
      /Package script validation failed/.test(validatePackageScripts) &&
      /referenced file does not exist/.test(validatePackageScripts) &&
      /npm --prefix/.test(validatePackageScripts) &&
      /"validate:scripts":\s*"node \.\/scripts\/validate-package-scripts\.mjs"/.test(pkg) &&
      /npm run test:live-data-audit && npm run validate:scripts && npm run test:snn/.test(pkg),
  },
  {
  ...
  },
  {
    ...
    ok:
      /api\/security\/totp\/enrollment/.test(bridge) &&
