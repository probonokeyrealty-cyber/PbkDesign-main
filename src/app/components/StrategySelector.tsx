    ...
    return [
      {
        ...
        tone: 'slate',
        description:
          'Builder-facing land workflow with offer-to-seller math and quick disposition timing.',
        timeline: '21-30 days',
        viable:
          deal.builderTotal > 0 || deal.offer > 0 || Number.parseFloat(deal.lotSize || '0') > 0,
      ...
      },
      {
        ...
        tone: 'amber',
        description:
          'Backup land path that keeps the retail-style leverage while protecting the seller conversation.',
        timeline: '21-30 days',
  ...
  return [
    {
      ...
      tone: 'green',
      description:
        'Use the fastest path when the seller wants certainty, speed, and an as-is exit.',
      timeline: '7-14 days',
    ...
    },
    {
      ...
      tone: 'blue',
      description:
        'Structured terms path for higher headline price, monthly spread, and flexible seller outcomes.',
      timeline: '14-30 days',
      viable: deal.price > 0 && deal.arv > 0,
      primaryLabel: 'Down Payment',
      primaryValue: formatCurrency(
        deal.cfDownPayment || Math.round((deal.agreedPrice || deal.price || 0) * 0.04)
      ),
    ...
    },
    {
      ...
      tone: 'purple',
      description:
        'Use the existing mortgage when the seller needs relief and the current loan gives the deal leverage.',
      timeline: '14-30 days',
    ...
    },
    {
      ...
      tone: 'amber',
      description:
        'Retail-buyer path with a stronger top-line price and cash fallback for seller confidence.',
      timeline: '14-21 days',
      viable: deal.price > 0 && deal.arv > 0,
...
function getUnderwritingRules(deal: DealData, strategies: StrategyCard[]) {
  const byPath: Record<PBKPath, Array<{ rule: string; value: string; pass: boolean }>> = {
    cash: [
      ...
      {
        rule: 'List price in range',
        value: 'Price at or below MAO Cash',
        pass: deal.price > 0 && deal.mao60 > 0 && deal.price <= deal.mao60,
      },
      {
        rule: 'Repairs captured',
        value: 'Mid repair estimate entered',
        pass: deal.repairs.mid >= 0,
      },
    ],
    cf: [
      {
        rule: 'Price captured',
        value: 'Seller number loaded',
        pass: deal.price > 0 || (deal.agreedPrice || 0) > 0,
      },
      {
        rule: 'Rate / term ready',
        value: 'CF rate and term present',
        pass: (deal.cfRate || 0) > 0 && (deal.cfTerm || 0) > 0,
      },
      {
        rule: 'Down payment ready',
        value: 'CF down payment captured',
        pass: (deal.cfDownPayment || 0) > 0,
      },
    ],
    mt: [
      {
        rule: 'Loan balance ready',
        value: 'Existing balance captured',
        pass: (deal.balance || 0) > 0 || (deal.mtBalanceConfirm || 0) > 0,
      },
      {
        rule: 'Rate ready',
        value: 'Existing interest rate captured',
        pass: (deal.rate || 0) > 0 || (deal.mtRateConfirm || 0) > 0,
      },
    ...
    rbp: [
      ...
      {
        rule: 'Price in range',
        value: 'Price at or below MAO RBP',
        pass: deal.price > 0 && deal.maoRBP > 0 && deal.price <= deal.maoRBP,
      },
      {
        rule: 'Cash backup ready',
        value: 'RBP cash alternative captured',
        pass: (deal.rbpCashAlternative || 0) > 0,
      },
    ],
    'land-owner': [
      {
        rule: 'Lot size ready',
        value: 'Lot size entered',
        pass: Number.parseFloat(deal.lotSize || '0') > 0 || (deal.landLotSizeSqFt || 0) > 0,
      },
      {
        rule: 'Builder pricing ready',
        value: 'Builder value loaded',
        pass: deal.builderTotal > 0 || deal.builderPrice > 0 || (deal.landPriceSqFt || 0) > 0,
      },
    ...
    'rbp-land': [
      ...
      {
        rule: 'Cash backup ready',
        value: 'RBP cash alternative entered',
        pass: (deal.rbpCashAlternative || 0) > 0,
      },
      {
        rule: 'Lot size ready',
        value: 'Lot size confirmed',
        pass: Number.parseFloat(deal.lotSize || '0') > 0 || (deal.landLotSizeSqFt || 0) > 0,
      },
    ],
  };
  ...
  return (
    <div className="pbk-analyzer-card bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 mb-3 shadow-sm">
      ...
      <div className="mb-4 rounded-2xl border border-blue-200 bg-blue-50 px-3 py-2.5 text-[11px] leading-5 text-blue-700 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-300">
        Tap any strategy card to load that exact path in <strong>Call Mode</strong>. Scripts, live
        inputs, and objections now stay in Call Mode instead of spilling back into Analyzer.
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    className={`w-10 h-10 ${tone.badge} rounded-xl flex items-center justify-center shadow-sm`}
                  >
                ...
                <div
                  className={`px-2 py-0.5 ${tone.badge} text-white rounded-full text-[8px] font-bold tracking-wide`}
                >
                  {strategy.badge}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div
                  className={`text-[10px] font-semibold ${strategy.viable ? tone.accent : 'text-amber-700 dark:text-amber-300'}`}
                >
                  {strategy.viable ? 'Ready for Call Mode' : 'Needs inputs'}
                </div>
                <div className="flex items-center gap-2">
                  <div
                    className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-semibold ${isSelected ? 'bg-white text-gray-900' : 'bg-white/70 text-gray-700 dark:bg-slate-900/70 dark:text-gray-200'}`}
                  >
                    ...
                    <div className="space-y-1.5">
                      {rules.map((rule) => (
                        <div key={rule.rule} className="flex items-start gap-2 text-[11px]">
                          <div
                            className={`mt-0.5 ${rule.pass ? 'text-green-500' : 'text-amber-500'}`}
                          >
                            {rule.pass ? 'OK' : 'NEEDS'}
                          </div>
      ...
      {showAllPaths && (
        <div className="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
          <div className="bg-gradient-to-r from-gray-800 to-slate-900 px-4 py-2">
            <div className="text-[12px] font-bold text-white">Path Comparison Snapshot</div>
          </div>
